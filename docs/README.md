# Documentation Index

Welcome to the MITM Network Interceptor documentation!

## Getting Started

- 📖 **[Quick Start Guide](QUICKSTART.md)** - Get up and running in 5 minutes
- 📦 **[Packaging Guide](PACKAGING.md)** - How to package and install the extension

## User Guides

- 🎯 **[Request Body Matching](BODY-MATCHING.md)** - Block or allow by request body content (NEW!)
- 💤 **[Idle Timeout](IDLE-TIMEOUT.md)** - Auto-stop proxy when inactive (NEW!)
- 🪟 **[Multi-Window Usage](MULTI-WINDOW.md)** - Using the extension with multiple VS Code windows
- 🗑️ **[Uninstall Guide](UNINSTALL.md)** - How to cleanly uninstall the extension
- 📋 **[Updates & Migration](UPDATES.md)** - Migration guide from YAML to JSON format

## Technical Documentation

- 💚 **[Health Check System](HEALTH-CHECK.md)** - How the self-healing mechanism works
- 🔄 **[Multi-Window Lifecycle](MULTI-WINDOW-LIFECYCLE.md)** - Detailed window lifecycle management
- 🔧 **[Development Guide](DEVELOPMENT.md)** - For contributors and developers
- 🧪 **[Testing Guide](TESTING.md)** - Comprehensive testing procedures
- ✅ **[Production Ready](PRODUCTION-READY.md)** - Production readiness checklist
- 📝 **[Pre-Publish Checklist](PRE-PUBLISH-CHECKLIST.md)** - Before publishing to marketplace

## Project Information

- 📝 **[Changelog](../CHANGELOG.md)** - Version history and changes
- 🚀 **[Release Process](RELEASE-PROCESS.md)** - How to create releases
- 🤝 **[Contributing](CONTRIBUTING.md)** - How to contribute to the project
- 📋 **[Code of Conduct](CODE_OF_CONDUCT.md)** - Community guidelines
- 🔒 **[Security Policy](SECURITY.md)** - Security and vulnerability reporting
- ⚖️ **[License](LICENSE)** - MIT License
- 🎨 **[Icon Credits](ICON-CREDITS.md)** - Icon design information

## Quick Links

- [Main README](../README.md) - Back to main documentation
- [GitHub Repository](https://github.com/ikirifqi/mitm-vscode)
- [Report Issues](https://github.com/ikirifqi/mitm-vscode/issues)

## Documentation Structure

```
docs/
├── README.md                    # This file
├── QUICKSTART.md               # 5-minute setup
├── DEVELOPMENT.md              # Development guide
├── TESTING.md                  # Testing procedures
├── PACKAGING.md                # Packaging instructions
├── MULTI-WINDOW.md             # Multi-window usage
├── MULTI-WINDOW-LIFECYCLE.md   # Window lifecycle details
├── HEALTH-CHECK.md             # Health check polling
├── UNINSTALL.md                # Uninstallation guide
├── UPDATES.md                  # Migration guide
├── SECURITY.md                 # Security policy
├── PRODUCTION-READY.md         # Production checklist
├── PRE-PUBLISH-CHECKLIST.md    # Publishing checklist
├── CHANGELOG.md                # Version history
├── CONTRIBUTING.md             # Contribution guidelines
├── CODE_OF_CONDUCT.md          # Community standards
├── ICON-CREDITS.md             # Icon information
└── LICENSE                     # MIT License
```

## Need Help?

1. Check the relevant guide above
2. Search [existing issues](https://github.com/ikirifqi/mitm-vscode/issues)
3. Read the [FAQ](#) (coming soon)
4. Open a [new issue](https://github.com/ikirifqi/mitm-vscode/issues/new)

---

**Happy intercepting!** 🛡️
